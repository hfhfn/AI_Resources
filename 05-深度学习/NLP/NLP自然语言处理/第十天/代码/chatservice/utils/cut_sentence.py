...
def _cut_sentence_by_word(sentence):
    ...
    if len(temp)>0:
        result.append(temp)
    result = [i for i in result if len(i.strip())>=1]
    return result
...
def _cut_sentence(sentence,use_stopwords,use_seg):
    ...
    if use_stopwords:
        if not use_seg:
            result = [i for i in result if i not in stopwords and len(i.strip())>0]
        else:
            result = [i for i in result if i[0] not in stopwords and len(i[0].strip())>0]
