...
- Excel实现RFM划分案例
  ...
  - 求出RFM的中值，例如中位数，用中值和用户的实际值进行比较，高于中值的为高，否则为低
...
- **RFM图形展示**
  ...
  ><font color='red'>输出结果：</font>
  >
  >![image-20200822212014030](img\pyecharts_rfm.png)
...
- **基于RFM分组结果的分析**
  ...
  我们打开导出的sales_rfm_score.xlsx，然后建立数据透视表，从Excel中得出的数据可以看出，与我们用Python代码分析的结果是一致的
